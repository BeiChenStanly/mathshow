...
class DotScene(InteractiveScene):
    def construct(self):
        ...
        tongshi = Tex(r"(\sum_{i = 0}^{n-1}{a_ix^i})\cdot (\sum_{i = 0}^{n-1}{b_ix^i}) = \sum_{k = 0}^{2n-2}{c_kx^k},\quad c_k=\sum_{\substack{i+j=k \\ i,j \geq 0}}a_ib_j=\sum_{i=0}^{k}a_ib_{k-i}").shift(2*DOWN)
        self.play(Write(tongshi))
        self.wait()
        self.play(FadeOut(tongshi))
        self.wait()
        ...
        self.wait()
        on_2=Tex(r"O(n^2)",t2c={r"O(n^2)":BLUE}).shift(UP+4.2*RIGHT)
        self.play(FadeIn(on_2))
        self.wait()
        self.play(FadeOut(polies),FadeOut(procedure),FadeOut(on_2),FadeOut(rec),FadeOut(bracex),FadeOut(labelx),FadeOut(bracey),FadeOut(labely),
                  FadeOut(result))
        ##example
...
class DanWeiGenScene(InteractiveScene):
    def construct(self):
        ...
        p1 = Tex(r"z^n=r^ne^{in\theta}=1").shift(0.8*UP+3.9*RIGHT)
        procedure = Tex(r"\left\{\begin{matrix}r^n=1\\n\theta = 2k\pi\end{matrix}\right.").shift(3.8*RIGHT+0.4*DOWN)
        conclution = Tex(r"\left\{\begin{matrix}r=1\\\theta = \frac{2\pi}{n}\cdot k\end{matrix}\right.").shift(1.9*DOWN+3.7*RIGHT)
        n=8
        ...
        property2 = Tex(r"\omega_n^{\frac{n}{2}+k}=-\omega_n^k(k=0,1,\dots,\frac{n}{2}-1)").shift(2.3*DOWN)
